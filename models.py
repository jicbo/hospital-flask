...
class User(UserMixin, db.Model):
    __tablename__ = 'user'  # Changed back to original
...
class Appointment(db.Model):
    __tablename__ = 'appointment'  # Changed back to original
    id = db.Column(db.Integer, primary_key=True)
    appointment_date = db.Column(db.Date, nullable=False)
    appointment_time = db.Column(db.Time, nullable=False)
    patient_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Updated foreign key
    doctor_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Updated foreign key
...
class MedicalRecord(db.Model):
    __tablename__ = 'medical_record'  # Changed back to original
    id = db.Column(db.Integer, primary_key=True)
    patient_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Updated foreign key
...
class Prescription(db.Model):
    __tablename__ = 'prescription'  # Changed back to original
    id = db.Column(db.Integer, primary_key=True)
    doctor_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Updated foreign key
    patient_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Updated foreign key
